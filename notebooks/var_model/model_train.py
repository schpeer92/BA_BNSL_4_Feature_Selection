...
def transform_adj_matrix(adj_matrix: np.matrix, var_list: list) -> np.matrix:
    ...
    trans_adj_matrix = trans_adj_matrix[pd.Series(var_list).str.contains("_shifted")][
        :, ~pd.Series(var_list).str.contains("_shifted")
    ]
    np.fill_diagonal(trans_adj_matrix, 1)
...
def train_predict_save(
    ...
    with open(
        f"{filepath_input}{dataset_name}_adj_matrix{normalization}.pkl", "rb"
    ) as input_file:
        adj_matrix = pickle.load(input_file)

    with open(
        f"{filepath_input}{dataset_name}_var_list{normalization}.pkl", "rb"
    ) as input_file:
        var_list = pickle.load(input_file)

    trans_adj_matrix = transform_adj_matrix(adj_matrix, var_list)

    # save mask for coeff matrix
    with open(
        f"{filepath_output}{dataset_name}_mask{normalization}.pkl", "wb"
    ) as filename:
    ...
    # train without mask
    var_model_mask = VAR_bn(df_train, p, trans_adj_matrix)
    var_model_mask.train_model_per_col()
    # train with mask
    var_model_no_mask = VAR_bn(df_train, p)
    var_model_no_mask.train_model_per_col()

    # save models
    with open(
        f"{filepath_output}{dataset_name}_var_mask{normalization}.pkl", "wb"
    ) as filename:
        pickle.dump(var_model_mask, filename, protocol=pickle.HIGHEST_PROTOCOL)

    with open(
        f"{filepath_output}{dataset_name}_var_no_mask{normalization}.pkl", "wb"
    ) as filename:
    ...
    # save predictions
    df_pred_mask.to_csv(
        f"{filepath_output}{dataset_name}_pred_mask{normalization}.csv", index=False
    )
    df_pred_no_mask.to_csv(
        f"{filepath_output}{dataset_name}_pred_no_mask{normalization}.csv", index=False
    )
